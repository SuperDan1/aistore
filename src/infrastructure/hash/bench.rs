...
use aistore::infrastructure::hash::{
    cityhash_64_hash, crc32_hash, fnv1a_hash, murmur3_hash, xxh64_hash,
};
...
pub fn bench_short_strings(c: &mut Criterion) {
    ...
    group.bench_function("fnv1a_hash", |b: &mut criterion::Bencher| {
        b.iter(|| fnv1a_hash(black_box(&s)))
    });
    group.bench_function("murmur3_hash", |b: &mut criterion::Bencher| {
        b.iter(|| murmur3_hash(black_box(&s)))
    });
    group.bench_function("xxh64_hash", |b: &mut criterion::Bencher| {
        b.iter(|| xxh64_hash(black_box(&s)))
    });
    group.bench_function("cityhash_64_hash", |b: &mut criterion::Bencher| {
        b.iter(|| cityhash_64_hash(black_box(&s)))
    });
    group.bench_function("crc32_hash", |b: &mut criterion::Bencher| {
        b.iter(|| crc32_hash(black_box(&s)))
    });
